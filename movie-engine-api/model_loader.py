...
class ModelLoader:
    ...
    def _load_data(self):
        ...
        try:
            ...
            links_local = self.data_dir / 'links.csv'
            links_s3 = 'processed/ml-100k/links.csv'
            logger.info("Loading links...")
            self.links_df = self._load_csv_file(links_local, links_s3)
            
            logger.info(f"Data loaded - {len(self.movies_df)} movies, {len(self.ratings_df)} ratings, {len(self.links_df)} links")
